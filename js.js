'use strict';
let EPS = 1e-7; // епсілон для роботи з дійсними
// підключення canvas
let canvas = document.querySelector("canvas");
let ctx = canvas.getContext("2d");
// генерація точок множини за допомогою нормального розподілу
const size = 100;
let pts = [];
function generateRandomPoints(amnt) {
    let pts = [];
    for (let i = 0; i < amnt; i++) {
        let x = Math.random() * canvas.width * 0.96 + 0.02 * canvas.width, y = Math.random() * canvas.height * 0.96 + 0.02 * canvas.height;
        pts.push([Math.round(x), Math.round(y)]);
    }
    return pts;
}
// функція, що знаходить трикутник, що містить всі точки множини
...
// обрахунок центру и радіусу описанної окружності навколо треикутника
...
// функція, що видаляє кратні ребра
...
// функція, що знаходить триангуляцію
function triangulate(points) {
    let n = points.length;
    if (n < 3)
        return []; // трикутників немає
    points = points.slice(0); // копія масиву
    // массив индексів, відсортованих за координатою Х
    let ind = [];
    for (let i = 0; i < n; i++)
        ind.push(i);
    ...
    // знаходимо трикутник, що містить всі точки, і вставлємо його в кінець масиву з вершинами
    ...
    for (let i = ind.length - 1; i >= 0; i--) {
        // перебираєм всі трикутники
        // якщо точка знаходиться всередині трикутника, то треба його видалити
        for (let j = cur_points.length - 1; j >= 0; j--) {
            // якщо точка зправа від описаної окружності, то трикутник перевіряти більше не потрібно
            // точки відсортовані і тому також будуть справа
            ...
            // якщо точка поза окружністю, то трикутник змінювати не потрібно
            let dy = points[ind[i]][1] - cur_points[j].y;
            if (dx * dx + dy * dy - cur_points[j].r > EPS) {
                continue;
            }
            // видаляємо трикутник і додаємо його стороні в список ребер
            edges.push(cur_points[j].a, cur_points[j].b, cur_points[j].b, cur_points[j].c, cur_points[j].c, cur_points[j].a);
            cur_points.splice(j, 1);
        }
        // видаляємо кратні ребра
        delete_multiples_edges(edges);
        // створюємо нові трикутники послідовно по списку ребер
        for (let j = edges.length - 1; j >= 0;) {
            let b = edges[j];
            j--;
            if (j < 0)
    ...
    // формиуємо масив з тріангуляцією
...
function drawPoints(pts) {
    ...
    // рисування тріангуляцій
    console.log(pts);
    ctx.strokeStyle = "black";
    let triangles = triangulate(pts);
    console.log('3ang', triangles);
    // рисування сгенерованих точок
...
drawPoints(generateRandomPoints(size));
